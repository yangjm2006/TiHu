...
- [9. 关注与粉丝](#9-关注与粉丝)
- [10. 私信](#10-私信)
- [11. 管理员用户管理](#11-管理员用户管理)
- [12. 前端实际调用接口清单](#12-前端实际调用接口清单)
...
- 图书封面和用户头像当前通过 JSON 字段传递字符串，不使用 multipart。前端上传本地图片后会转成 `data:image/...;base64,...`；后端也可以存储为图片 URL，但返回给前端时字段值必须是可直接展示的 data URI 或 URL。空字符串或 `null` 表示默认空白图片。
...
接口传输时间字段推荐使用 ISO-8601 本地时间字符串：

```text
2026-05-31T12:00:00
```

前端会用 `LocalDateTime.parse` 解析，并在所有页面统一显示为：
...
后端不要在返回给前端的时间字符串里附带小数秒。如果后端返回 ISO 时间中带 `T`，前端展示时会去掉 `T` 并按上面的格式显示。
...
前端会显示：`您已被封禁，解封时间是 2026-06-02 12:00:00`。解封时间字段兼容 `bannedUntil`、`banExpireTime`、`until`、`unbanTime`；即使后端返回 ISO 时间，前端也会格式化为 `yyyy-MM-dd HH:mm:ss` 展示。
...
- 前端主要字段：`avatarImage`
- 兼容字段：`avatar`、`avatarUrl`
- 清空头像时前端会发送空字符串；后端应保存为空并在后续查询中返回空值。

## 3. 图书

### 3.1 图书列表

`GET /books?page=1&size=9&sort=default&keyword=三体&title=三体&tags=科幻&tags=宇宙`

图书列表、普通用户图书搜索、管理员图书管理都使用该接口。图书列表页每页展示 9 本书；管理员图书管理页会按管理页面自身需要请求分页。后端应按查询条件、排序、分页参数返回当前页数据。
...
```text
GET /books?page=1&size=9&sort=default
GET /books?page=1&size=9&sort=default&keyword=三体&title=三体
GET /books?page=1&size=9&sort=default&tags=科幻&tags=宇宙
GET /books?page=2&size=9&sort=rating_desc&keyword=三体&title=三体&tags=科幻
```
...
- 封面：`coverImage`、`cover`、`coverUrl`、`image`、`imageUrl`
...
- 图书封面：`coverImage`、`cover`、`coverUrl`、`image`、`imageUrl`，可放在图书对象顶层或嵌套图书对象中
...
- 前端主要字段：`coverImage`
- 兼容字段：`cover`、`coverUrl`
- 清空封面时前端会发送空字符串；后端应保存为空并在图书详情中返回空值。
...
图书详情页加入书单时，前端已知当前图书 ID，会使用 `bookId`。书单详情页用户输入图书名称时，前端会先按完整书名精确匹配图书；只有书名完全一致时才解析为 `bookId` 并调用 `POST /book-lists/{listId}/books?bookId=...`。找不到精确匹配时前端显示“书名不存在”，不会用模糊匹配结果加入书单。

后端如继续支持 `bookTitle` 兼容接口，也必须按完整书名精确匹配，不应按模糊搜索自动加入。
...
- 头像：`avatarImage`、`avatar`、`avatarUrl`、`profileImage`、`profileImageUrl`
...
`GET /users/admin?page=1&size=1000&sort=created_at_asc`

管理员用户管理页使用该接口展示所有用户。后端应按注册时间排序，推荐 `created_at_asc` 表示注册时间从旧到新，即最早注册的用户在最上面。
...
- 用户名：`username`、`user`、`name`，也兼容嵌套 `userInfo`、`user`、`profile`、`account`
- 角色：`role`、`userRole`，值为 `USER` / `ADMIN`
- 注册时间：`createdAt`、`createTime`、`registerTime`、`registeredAt`、`registrationTime`
...
该接口作为兼容接口保留。当前用户管理页优先使用 `GET /users/admin` 展示全部用户。
...
成功响应：通用成功响应即可。后端必须把该用户持久化更新为正常状态，并清空封禁截止时间：

- `status = 0`
- `bannedUntil` / `banExpireTime` / `until` / `unbanTime` 对应的数据库字段应写为 `NULL`

解封后再次查询 `GET /users/admin` 时，该用户的封禁时间字段应返回 `null`，前端会显示“正常”。

前端支持从用户列表选中用户后直接解除封禁。

### 11.5 赋予管理员权限

`POST /users/grant-admin?username=alice`

成功响应：通用成功响应即可。后端应把该用户角色更新为 `ADMIN`，再次查询 `GET /users/admin` 时返回 `role: "ADMIN"`。
